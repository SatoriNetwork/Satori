defmodule Satori.WalletsTest do
  ...
  describe "wallets" do
    ...
    @invalid_attrs %{address: nil, pubkey: nil, user_id: nil}
    ...
    test "create_wallet/1 with valid data creates a wallet" do
      valid_attrs = %{address: "some address", pubkey: "some pubkey", user_id: 42}

      assert {:ok, %Wallet{} = wallet} = Wallets.create_wallet(valid_attrs)
      assert wallet.address == "some address"
      assert wallet.pubkey == "some pubkey"
    ...
    test "update_wallet/2 with valid data updates the wallet" do
      wallet = wallet_fixture()
      update_attrs = %{address: "some updated address", pubkey: "some updated pubkey", user_id: 43}

      assert {:ok, %Wallet{} = wallet} = Wallets.update_wallet(wallet, update_attrs)
      assert wallet.address == "some updated address"
      assert wallet.pubkey == "some updated pubkey"
